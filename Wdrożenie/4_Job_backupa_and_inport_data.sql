...
/****** Object:  Job [ServiceMap Backup and Import Data]    Script Date: 2017-05-29 23:31:57 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]    Script Date: 2017-05-29 23:31:57 ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'ServiceMap Backup and Import Data', 
		@enabled=1, 
		...
		@owner_login_name=N'WWW\mariohyla', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Backup]    Script Date: 2017-05-29 23:31:57 ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Backup', 
		...
		@command=N'-- Backup z 2 ostatnich dni
Declare 
@DayOfweek varchar(2),
@sqlBackUp nvarchar(max)='''';
SELECT @DayOfweek=TRY_CONVERT(nvarchar(2), (DATEPART(dw, GETDATE())%2));
select @sqlBackUp=N''C:\App_ServiceMap\BackupDB\ServiceMap_''+ @DayOfweek+''.bak''
select @sqlBackUp

BACKUP DATABASE [ServiceMap] TO  DISK = @sqlBackUp WITH NOFORMAT, INIT,  NAME = ''ServiceMap-Full Database Backup'', SKIP, NOREWIND, NOUNLOAD,  STATS = 10

GO

-- Backup z 2 ostatnich dni
Declare 
@DayOfweek varchar(2),
@sqlBackUp nvarchar(max)='''';
SELECT @DayOfweek=TRY_CONVERT(nvarchar(2), (DATEPART(dw, GETDATE())%2));

select @sqlBackUp=N''C:\App_ServiceMap\BackupDB\msdb_''+ @DayOfweek+''.bak''
select @sqlBackUp

BACKUP DATABASE [msdb] TO  DISK = @sqlBackUp WITH NOFORMAT, INIT,  NAME = N''msdb-Full Database Backup'', SKIP, NOREWIND, NOUNLOAD,  STATS = 10

GO

-- Backup z 7 ostatnich dni
Declare 
@DayOfweek varchar(2),
@sqlBackUp nvarchar(max)='''';
SELECT @DayOfweek=TRY_CONVERT(nvarchar(2), DATEPART(dw, GETDATE()));

select @sqlBackUp=N''C:\App_ServiceMap\BackupDB\SM_IdentityUser_''+ @DayOfweek+''.bak''
select @sqlBackUp

BACKUP DATABASE [SM_IdentityUser] TO  DISK = @sqlBackUp WITH NOFORMAT, INIT,  NAME = N''SM_IdentityUser-Full Database Backup'', SKIP, NOREWIND, NOUNLOAD,  STATS = 10
...
', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Download files from ftp]    Script Date: 2017-05-29 23:31:57 ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Download files from ftp', 
		...
		@command=N'C:\App_ServiceMap\ImportDanych\mrowa.cmd', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [ImportServicesTnt]    Script Date: 2017-05-29 23:31:57 ******/
...
BEGIN TRY 
	...
	FROM ''C:\App_ServiceMap\ImportDanych\downloaded_files\mapa_serwisowa_na_www.txt'' WITH (FIRSTROW=2, FORMATFILE=''C:\App_ServiceMap\ImportDanych\fmt\ServiceTnt.fmt'');  
END TRY
BEGIN CATCH
	THROW
END CATCH
...
END;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [ImportLocation]    Script Date: 2017-05-29 23:31:57 ******/
...
BEGIN TRY 
	...
	FROM ''C:\App_ServiceMap\ImportDanych\downloaded_files\mapa_serwisowa_na_www_lokalizacja.txt'' WITH (FIRSTROW=2, FORMATFILE=''C:\App_ServiceMap\ImportDanych\fmt\Location.fmt'', CODEPAGE = 65001);  
END TRY
BEGIN CATCH
	THROW
END CATCH
...
END;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Shrink Database]    Script Date: 2017-05-29 23:31:57 ******/
...
GO
ALTER DATABASE [ServiceMap] SET RECOVERY SIMPLE WITH NO_WAIT
GO

USE [ServiceMap]
GO
DBCC SHRINKDATABASE(N''ServiceMap'' )
GO

USE [master]
GO
ALTER DATABASE [ServiceMap] SET RECOVERY FULL WITH NO_WAIT
GO
', 
		@database_name=N'master', 
		@flags=0
